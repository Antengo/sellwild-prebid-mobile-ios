...
@property (nonatomic, readonly, nonnull) SWPBMTransactionFactoryCallback callbackForProperFactory;

@end

@implementation SWPBMTransactionFactory_Objc
...
- (BOOL)loadHTMLTransaction:(NSString *)adMarkup {
    SWPBMDisplayTransactionFactory * const factory = [[SWPBMDisplayTransactionFactory alloc] initWithBid:self.bid
...
- (BOOL)loadVASTTransaction:(NSString *)adMarkup {
    SWPBMVastTransactionFactory * const factory = [[SWPBMVastTransactionFactory alloc] initWithBid:self.bid
                                                                                    connection:self.connection
...
- (SWPBMTransactionFactoryCallback)callbackForProperFactory {
    @weakify(self);
    return ^(id<SWPBMTransaction> _Nullable transaction, NSError * _Nullable error) {
        @strongify(self);
        if (!self) { return; }
