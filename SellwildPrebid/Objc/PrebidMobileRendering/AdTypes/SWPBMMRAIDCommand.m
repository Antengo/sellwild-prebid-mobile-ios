...
@interface SWPBMMRAIDCommand ()

@property (nonatomic, copy, nonnull) SWPBMMRAIDAction command;
...
@implementation SWPBMMRAIDCommand
...
- (nullable instancetype)initWithURL:(nonnull NSString *)url error:(NSError* _Nullable __autoreleasing * _Nullable)error {
    ...
    if (!url) {
        ...
        [SWPBMError createError:error description:message];
        return nil;
    }
    
    NSString *mraidPath = [url SWPBMsubstringFromString:[SWPBMMRAIDConstants mraidURLScheme]];
    if (!mraidPath) {
    ...
    for (NSString *constant in [SWPBMMRAIDConstants allCases]) {
