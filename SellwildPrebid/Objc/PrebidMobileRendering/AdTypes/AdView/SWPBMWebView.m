...
#import "SWPBMWKScriptMessageHandlerLeakAvoider.h"
#import "UIView+SWPBMExtensions.h"
#import "SWLog+Extensions.h"

#import "SWPBMWebView.h"
#import "SWPBMWebView+Internal.h"

#import "SWSwiftImport.h"

#pragma mark - Constants

static NSString * const SWPBMInternalWebViewAccessibilityIdentifier = @"SWPBMInternalWebViewAccessibilityIdentifier";
typedef BOOL(^SWPBMIsVisibleViewBlock)(UIView *, UIView *);

static NSString * const KeyPathOutputVolume = @"outputVolume";

#pragma mark - Private Extension

@interface SWPBMWebView ()
...
// viewability polling
@property (nonatomic, strong, nullable) id<SWPBMCreativeViewabilityTracker> viewabilityTracker;
                            ...
                            targeting:(Targeting *)targeting
{
    ...
    _state = SWPBMWebViewStateLoading;
    self.mraidState = SWPBMMRAIDState.notEnabled;
    _mraidLastSentFrame = CGRectZero;
    _bundle = [SWPBMFunctions bundleForSDK];
    ...
    [wkUserContentController addScriptMessageHandler:[[SWPBMWKScriptMessageHandlerLeakAvoider alloc] initWithDelegate:self] name:@"log"];
    ...
    internalWebView.accessibilityIdentifier = SWPBMInternalWebViewAccessibilityIdentifier;
    
    [self addSubview:self.internalWebView];
    
    [internalWebView SWPBMAddFillSuperviewConstraints];
...
/**
 ...
 This function is used by SWPBMHTMLCreative to load html ads for both .auid and .html AdUnitIdentifierTypes
...
- (void)loadHTML:(nonnull NSString *)html
   ...
   currentThread:(id<SWPBMThreadProtocol>)currentThread {
    if (!html) {
        SWPBMLogError(@"Input HTML is nil");
        return;
    }
    
    if (!currentThread.isMainThread) {
        SWPBMLogError(@"Attempting to loadHTML on background thread");
    }
    ...
    [self loadContentWithMRAID:injectMraidJs forExpandContent:NO contentLoader:^{
        ...
        SWPBMLogInfo(@"loadHTMLString");
        self.state = SWPBMWebViewStateLoading;
...
/**
 ...
 This function is used by SWPBMHTMLCreative to support MRAID.expand
...
- (void)expand:(nonnull NSURL *)url currentThread:(id<SWPBMThreadProtocol>)currentThread {
    if (!url) {
        SWPBMLogError(@"Could not expand with nil url");
        return;
    }
...
+ (BOOL)isVisibleView:(UIView *)view {
    ...
    return [view swpbmIsVisibleInView:view.superview] && view.window != nil;
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction
    ...
    if (!url || url.absoluteString.length == 0) {
        SWPBMLogWarn(@"No URL found on WKWebView navigation");
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    }
    
    //Identify and process MRAID links
    if ([SWPBMMRAIDController isMRAIDLink:url.absoluteString]) {
        self.isMRAID = YES;
        @weakify(self);
        dispatch_async(dispatch_get_main_queue(), ^{
            @strongify(self);
    ...
    if (self.state == SWPBMWebViewStateLoading) {
        decisionHandler(WKNavigationActionPolicyAllow);
        return;
    }

    //Bail if the state is uninitialized, unloaded, or still loading.
    if (self.state != SWPBMWebViewStateLoaded) {
        SWPBMLogWarn(@"Unexpected state [%@] found on navigation to url: %@", [SWPBMWebView webViewStateDescription:self.state], url);
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    }
    ...
    if ([self wasRecentlyTapped]) {
        ...
        SWPBMLogWarn(@"User has not recently tapped. Auto-click suppression is preventing navigation to: %@", url);
    }
...
- (void)pollForDocumentReadyState {
    if (self.isPollingForDocumentReady || self.state == SWPBMWebViewStateLoaded) {
        return;
    }
...
- (void)checkDocumentReadyState {
    ...
    [self.internalWebView evaluateJavaScript:@"document.readyState" completionHandler:^(NSString * _Nullable readyState, NSError * _Nullable error) {
        ...
        if ([readyState isEqualToString:@"complete"]) {
            self.state = SWPBMWebViewStateLoaded;
...
static SWPBMError *extracted(NSString *errorMessage) {
    return [SWPBMError errorWithMessage:errorMessage type:SWPBMErrorType.internalError];
}

- (void)webView:(WKWebView *)webView didFailNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    SWPBMLogWhereAmI();
    self.state = SWPBMWebViewStateUnloaded;
    ...
    SWPBMError *prebidError = extracted(errorMessage);
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    SWPBMLogInfo(@"JS: %@", (NSString *)message.body ?: @"");
}
...
- (BOOL)injectMRAIDForExpandContent:(BOOL)isForExpandContent error:(NSError **)error {
    ...
    if (!mraidScript) {
        [SWPBMError createError:error message:@"Could not load mraid.js from library manager" type:SWPBMErrorType.internalError];
        return false;
    }
    ...
    [self.internalWebView evaluateJavaScript:mraidScript completionHandler:^(id _Nullable jsRet, NSError * _Nullable error) {
        ...
        if (error) {
            SWPBMLogError(@"Error injecting MRAID script: %@", error);
            return;
        }
        ...
        NSString *command = isForExpandContent ?
            [SWPBMMRAIDJavascriptCommands onReadyExpanded] :
            [SWPBMMRAIDJavascriptCommands onReady];
        ...
        [self.internalWebView evaluateJavaScript:command completionHandler:^(id _Nullable javaScriptString, NSError * _Nullable error) {
            ...
            if (error) {
                SWPBMLogError(@"Error calling %@: %@", command, error.localizedDescription);
                return;
            }

            self.mraidState = isForExpandContent ? SWPBMMRAIDState.expanded : SWPBMMRAIDState.defaultState;
        }];
    }];
...
- (void)injectMRAIDEnvAndExecute:(nonnull SWPBMVoidBlock)completionBlock onError:(void (^_Nullable)(NSError * _Nonnull error))onError {
...
- (NSString *)buildMraidEnvObject {
    ...
    nextFeed(); [s appendFormat:@"sdkVersion: '%@'", [SWPBMFunctions sdkVersion]];
...
- (void)loadContentWithMRAID:(BOOL)injectMRAID forExpandContent:(BOOL)forExpandContent contentLoader:(nonnull SWPBMVoidBlock)contentLoader onError:(void (^_Nullable)(NSError * _Nullable error))onError {
    if (injectMRAID) {
        @weakify(self);
        SWPBMVoidBlock injectMraidLib = ^{
            @strongify(self);
            if (!self) { return; }
...
- (void)updateMRAIDLayoutInfoWithForceNotification:(BOOL)forceNotification {
    //SWPBMLogInfo(@"MRAID_updateLayoutInfo HAS BEEN CALLED");
    [self MRAID_updateCurrentPosition:self.frame forceNotification:forceNotification];
    //self.MRAID_onViewableChange(SWPBMFunctions.isVisible(self))
}

- (void)changeToMRAIDState:(SWPBMMRAIDState *)state {
...
- (void)MRAID_nativeCallComplete {
    NSString *command = [SWPBMMRAIDJavascriptCommands nativeCallComplete];
    if (NSThread.isMainThread) {
        WKUserScript *script = [[WKUserScript alloc] initWithSource:command injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
        [self.internalWebView.configuration.userContentController addUserScript:script];
        [self.internalWebView evaluateJavaScript:command completionHandler:^(id _Nullable jsRet, NSError * _Nullable error) {
            if (error) {
                SWPBMLogError(@"Error of executing command %@", command);
            }
        }];
    } else {
...
- (void)MRAID_onSizeChange {
    ...
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands onSizeChange:self.frame.size]];
...
- (void)MRAID_onStateChange:(SWPBMMRAIDState *)state {
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands onStateChange:state]];
}

// updates the viewable flag in mraid.js
- (void)MRAID_onExposureChange:(id<SWPBMViewExposure>)viewExposure {
    BOOL const newViewable = viewExposure.exposureFactor > 0;
    if (self.isViewable != newViewable) {
        self.viewable = newViewable;
        [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands onViewableChange:newViewable]];
    }
    
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands onExposureChange:viewExposure]];
}
...
- (void) setSupportedMRAIDFeatures {
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateSupportedFeatures]];
}
...
- (void)MRAID_updatePlacementType:(SWPBMMRAIDPlacementType)type {
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updatePlacementType:type]];
}
...
- (void)MRAID_updateScreenSize {
    ...
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateScreenSize:screenSize]];
}

// update the maximum size of the device screen in mraid.js
- (void)MRAID_updateMaxSize {
    CGSize maxSize = [SWPBMFunctions deviceMaxSize];
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateMaxSize:maxSize]];
}

// This method sets default (starting) position for the webview in mraid.js
- (void)MRAID_updateDefaultPosition:(CGRect)position {
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateDefaultPosition:position]];
}
...
- (void)MRAID_updateCurrentPosition:(CGRect)position forceNotification:(BOOL)forceNotification {
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateCurrentPosition:position]];
    [self MRAID_onSizeChange];
}

- (void)MRAID_updateLocation {
    if (SellwildPrebid.shared.locationUpdatesEnabled && SWPBMLocationManager.shared.coordinatesAreValid) {
        SWPBMLocationManager *locationManager = SWPBMLocationManager.shared;
        [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands updateLocation:locationManager.coordinates
...
- (void)MRAID_getExpandProperties:(void(^)(SWPBMMRAIDExpandProperties *))completionHandler {
    ...
    NSString *command = [SWPBMMRAIDJavascriptCommands getExpandProperties];
    WKUserScript *script = [[WKUserScript alloc] initWithSource:command injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
    [self.internalWebView.configuration.userContentController addUserScript:script];
    [self.internalWebView evaluateJavaScript:command completionHandler:^(id _Nullable jsRet, NSError * _Nullable error) {
        if (error) {
            SWPBMLogError(@"Error getting expand properties: %@", error.localizedDescription);
            completionHandler(nil);
            return;
        }
        
        SWPBMJsonDictionary *dict = [SWPBMWebView anyToJSONDict:jsRet];
        if (!dict) {
            completionHandler(nil);
            return;
        }
        
        NSNumber *width = dict[SWPBMMRAIDParseKeys.WIDTH];
        NSNumber *height = dict[SWPBMMRAIDParseKeys.HEIGHT];
        if (width && height) {
            SWPBMMRAIDExpandProperties *mraidExpandProperties = [[SWPBMMRAIDExpandProperties alloc] initWithWidth:width.integerValue
...
- (void) MRAID_getResizeProperties:(void(^)(SWPBMMRAIDResizeProperties *))completionHandler {
    if (!completionHandler) {
        SWPBMLogError(@"The completionHandler is not provided");
        return;
    }
    
    NSString *command = [SWPBMMRAIDJavascriptCommands getResizeProperties];
    WKUserScript *script = [[WKUserScript alloc] initWithSource:command injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
    [self.internalWebView.configuration.userContentController addUserScript:script];
    [self.internalWebView evaluateJavaScript:command completionHandler:^(id _Nullable jsRet, NSError * _Nullable error) {
        if (error) {
            SWPBMLogError(@"Error getting Resize Properties: %@", error.localizedDescription);
            completionHandler(nil);
            return;
        }
        ...
        NSNumber *width = dict[SWPBMMRAIDParseKeys.WIDTH];
        NSNumber *height = dict[SWPBMMRAIDParseKeys.HEIGHT];
        NSNumber *offsetX = dict[SWPBMMRAIDParseKeys.X_OFFSET];
        NSNumber *offsetY = dict[SWPBMMRAIDParseKeys.Y_OFFSET];
        NSNumber *allowOffscreen = dict[SWPBMMRAIDParseKeys.ALLOW_OFFSCREEN];
        
        if (width && height && offsetX && offsetY && allowOffscreen) {
            SWPBMMRAIDResizeProperties *mraidResizeProperties = [[SWPBMMRAIDResizeProperties alloc] initWithWidth:width.integerValue
                                                                                                       height:height.integerValue
...
- (void)MRAID_error:(NSString *)message action:(SWPBMMRAIDAction)action {
    SWPBMLogError(@"Action: [%@] generated error with message [%@]", action, message);
    [self evaluateJavaScript:[SWPBMMRAIDJavascriptCommands onErrorWithMessage:message action: action]];
}
...
- (void)pollForViewability {
    @weakify(self);
    self.viewabilityTracker = [SWPBMFactory createCreativeViewabilityTrackerWithView:self pollingTimeInterval:0.2f onExposureChange:^(id<SWPBMCreativeViewabilityTracker> tracker, id<SWPBMViewExposure> _Nonnull viewExposure) {
        @strongify(self);
        if (!self) { return; }
...
- (void)setupTapRecognizer {
    ...
    self.tapdownGestureRecognizer = [[SWPBMTouchDownRecognizer alloc] initWithTarget:self action:@selector(recordTapEvent:)];
...
- (void)onStatusBarOrientationChanged {    
    ...
    SWPBMLogInfo(@"Orientation is portrait: %@", isPortrait);
...
- (void)addFriendlyObstructionsToMeasurementSession:(SWPBMOpenMeasurementSession *)session {
}

#pragma mark - Utilities


+ (NSString *)webViewStateDescription:(SWPBMWebViewState)state {
    switch (state) {
        case SWPBMWebViewStateUnloaded        : return @"unloaded";
        case SWPBMWebViewStateLoading         : return @"loading";
        case SWPBMWebViewStateLoaded          : return @"loaded";
    }
}

+ (SWPBMJsonDictionary *)anyToJSONDict:(id)str {
    ...
    if (![jsonObj isKindOfClass:[SWPBMJsonDictionary class]]) {
        return nil;
    }

    return (SWPBMJsonDictionary *)jsonObj;
}
