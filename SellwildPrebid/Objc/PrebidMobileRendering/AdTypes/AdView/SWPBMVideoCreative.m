...
@interface SWPBMVideoCreative ()

@property (nonatomic, strong) SWPBMVideoView *videoView;
...
@implementation SWPBMVideoCreative
...
- (instancetype)initWithCreativeModel:(SWPBMCreativeModel *)creativeModel
    ...
    if (self) {
        ...
        self.videoView = [[SWPBMVideoView alloc] initWithCreative:self];
...
- (void)displayWithRootViewController:(UIViewController *)viewController {
    ...
    [self.videoView SWPBMAddFillSuperviewConstraints];
}
...
- (void)showAsInterstitialFromRootViewController:(UIViewController *)uiViewController displayProperties:(SWPBMInterstitialDisplayProperties *)displayProperties {
    ...
    SWPBMInterstitialDisplayProperties* newDisplayProperties = [self createInterstitialPropertiesForCurrentVideoStateFor:displayProperties];
...
// Should the concept of "close" or dismiss be moved to SWPBMAbstractCreative?
- (void)close {
    [self.videoView stopOnCloseButton:SWPBMTrackingEventCloseLinear];
}
...
- (void)createOpenMeasurementSession {
    ...
    if (self.transaction.measurementSession && [self.transaction.measurementSession isKindOfClass:SWPBMOpenMeasurementSession.class]) {
        ...
        [self.eventManager trackVideoAdLoaded:[SWPBMVideoVerificationParameters new]];
    }
}

#pragma mark - SWPBMVideoViewDelegate
...
- (void)videoViewCompletedDisplay {
    ...
    if (self.creativeModel.adConfiguration.presentAsInterstitial) {
        // no companion ads so pass this event to the SWPBMModalManager
...
- (SWPBMInterstitialDisplayProperties *)createInterstitialPropertiesForCurrentVideoStateFor:(SWPBMInterstitialDisplayProperties *)initialProperties {
    SWPBMInterstitialDisplayProperties *newDisplayProperties = [initialProperties copy];
