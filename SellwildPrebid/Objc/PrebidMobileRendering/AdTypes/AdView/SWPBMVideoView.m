...
static NSString * const SWPBMAVPlayerObserverKeyStatus        = @"status";
static NSString * const SWPBMAVPlayerObserverKeyVolume        = @"volume";
static NSString * const SWPBMAudioSessionObserverKeyVoulume   = @"outputVolume";

static NSString * const SWPBMLearnMoreButtonTitle             = @"Learn More";
static NSString * const SWPBMWatchAgainButtonTitle            = @"Watch Again";
...
@interface SWPBMVideoView () <UIGestureRecognizerDelegate>

#pragma mark Model

@property (nonatomic, weak) SWPBMVideoCreative *creative;
@property (nonatomic, strong) SWPBMEventManager *eventManager;
@property (nonatomic, strong) SWPBMTouchDownRecognizer *tapdownGestureRecognizer;
...
@property (nonatomic, strong) SWPBMAdViewButtonDecorator *skipButtonDecorator;
...
- (SWPBMAdConfiguration *)adConfiguration {
    return self.creative.creativeModel.adConfiguration;
}

#pragma mark - Initialization

- (instancetype)initWithEventManager:(SWPBMEventManager *)eventManager {
    self = [super init];
    if (self) {
...
- (instancetype)initWithCreative:(SWPBMVideoCreative *)creative {
...
- (void)dealloc {
    ...
    if (self.avPlayer) {
        [self.avPlayer.currentItem removeObserver:self forKeyPath:SWPBMAVPlayerObserverKeyStatus];
        [self.avPlayer removeObserver:self forKeyPath:SWPBMAVPlayerObserverKeyVolume];
        ...
        [AVAudioSession.sharedInstance removeObserver:self forKeyPath:SWPBMAudioSessionObserverKeyVoulume];
    }
}

- (void)setupWithEventManager:(SWPBMEventManager *)eventManager {
    self.isPlaybackFinished = NO;
    self.showLearnMore = NO;
    self.isPlaybackStarted = NO;
    self.eventManager = eventManager;
    self.accessibilityIdentifier = @"SWPBMVideoView";
...
- (void)showMediaFileURL:(NSURL *)mediaFileURL preloadedData:(NSData *)preloadedData {
    SWPBMLogWhereAmI();
    SWPBMAssert(mediaFileURL && preloadedData);
    if (!(mediaFileURL )) {
        return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self.avPlayer addObserver:self forKeyPath:SWPBMAVPlayerObserverKeyVolume options:NSKeyValueObservingOptionNew context:nil];
    });
...
- (void)updateLearnMoreButtonVisibility {
    ...
    SWPBMCreativeModel *creativeModel = self.creative.creativeModel;
...
- (void)setupObserversWithPlayerItem:(AVPlayerItem *)playerItem {
    ...
    [playerItem addObserver:self forKeyPath:SWPBMAVPlayerObserverKeyStatus options:NSKeyValueObservingOptionInitial context:nil];
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest {
    if (!self.preloadedData) {
        [loadingRequest finishLoadingWithError:[SWPBMError errorWithDescription:@"data was not pre-fetched"]];
        return false;
    }
    
    AVAssetResourceLoadingDataRequest *dataRequest = loadingRequest.dataRequest;
    if (!dataRequest) {
        [loadingRequest finishLoadingWithError:[SWPBMError errorWithDescription:@"No data request"]];
        return false;
    }
    
    NSUInteger dataLength = self.preloadedData ? self.preloadedData.length : 0;
    if (dataLength == 0) {
        [loadingRequest finishLoadingWithError:[SWPBMError errorWithDescription:@"preloadedData is empty!"]];
        return false;
    }
    ...
    if (end > dataLength) {
        ...
        [loadingRequest finishLoadingWithError:[SWPBMError errorWithDescription:message]];
        return false;
    }
...
- (void)updateLearnMoreButton {
    ...
    self.btnLearnMore = [self createButtonWithTitile:SWPBMLearnMoreButtonTitle action:@selector(btnLearnMoreClick)];
    [self addSubview:self.btnLearnMore];

    [self.btnLearnMore SWPBMAddBottomRightConstraintsWithMarginSize:CGSizeMake(-25.0, -25.0)];
}
...
- (void)updateProgressBar {
    ...
    SWPBMCircularProgressBarView *progressBar = [[SWPBMCircularProgressBarView alloc] initWithFrame:CGRectMake(30,60, 36, 36)];
    ...
    [progressBar SWPBMAddBottomLeftConstraintsWithViewSize:CGSizeMake(36, 36) marginSize:CGSizeMake(25.0, -25.0)];
...
- (void)updateWatchAgainButton {
    ...
    self.btnWatchAgain = [self createButtonWithTitile:SWPBMWatchAgainButtonTitle action:@selector(btnWatchAgainClick)];
    [self addSubview:self.btnWatchAgain];
    
    [self.btnWatchAgain SWPBMAddCropAndCenterConstraintsWithInitialWidth:self.btnWatchAgain.frame.size.width initialHeight:self.btnWatchAgain.frame.size.height];
}
...
- (void)setupSkipButton {
    self.skipButtonDecorator = [SWPBMAdViewButtonDecorator new];
...
- (void)skipButtonTapped {
    ...
    [self.avPlayer pause];
    [self completeVideoViewDisplayWith:SWPBMTrackingEventSkip];
}
...
- (void)handleSkipDelay:(NSTimeInterval)skipDelay videoDuration:(NSTimeInterval)videoDuration {
    ...
    dispatch_time_t dispatchTime = [SWPBMFunctions dispatchTimeAfterTimeInterval:self.creative.creativeModel.adConfiguration.videoControlsConfig.skipDelay];
    @weakify(self);
...
- (void)startPlayback {
    ...
    if (!self.avPlayer) {
        SWPBMLogError(@"Attempted to display a VideoView with no avPlayer");
        return;
    }
...
- (void)pause {
    if (!self.avPlayer) {
        SWPBMLogError(@"Attempted to pause a VideoView with no avPlayer");
        return;
    }
    ...
    [self.avPlayer pause];
    [self.eventManager trackEvent:SWPBMTrackingEventPause];
...
- (void)resume {
    ...
    [self.avPlayer play];
    [self.eventManager trackEvent:SWPBMTrackingEventResume];
...
- (void)stop {
    [self stopWithTrackingEvent:SWPBMTrackingEventSkip];
}

- (void)stopWithTrackingEvent:(SWPBMTrackingEvent)trackingEvent {
    if (!self.avPlayer) {
        SWPBMLogError(@"No AVPlayer to stop");
        return;
    }
...
- (void)stopOnCloseButton:(SWPBMTrackingEvent)trackingEvent {
    if (!self.avPlayer) {
        SWPBMLogError(@"No AVPlayer to stop");
        return;
    }
...
- (void)addFriendlyObstructionsToMeasurementSession:(SWPBMOpenMeasurementSession *)session {
    [session addFriendlyObstruction:self.btnLearnMore purpose:SWPBMOpenMeasurementFriendlyObstructionPurposeVideoViewLearnMoreButton];
    [session addFriendlyObstruction:self.progressBar purpose:SWPBMOpenMeasurementFriendlyObstructionPurposeVideoViewProgressBar];
}

#pragma mark - Observers

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:SWPBMAVPlayerObserverKeyStatus]) {
        [self onPlayerStatusChanged];
    }
    else if ([keyPath isEqualToString:SWPBMAVPlayerObserverKeyVolume] && object == self.avPlayer) {
        [self onPlayerVolumeChanged];
    }
    else if ([keyPath isEqualToString:SWPBMAudioSessionObserverKeyVoulume]) {
...
- (void)completeVideoViewDisplayWith:(SWPBMTrackingEvent)trackingEvent {
...
- (void)handleDidPlayToEndTime {
    ...
    [self completeVideoViewDisplayWith:SWPBMTrackingEventComplete];
}
...
- (CGFloat)handlePeriodicTimeEvent {
    ...
    if (CMTimeCompare(endTime, kCMTimeZero)) {
        ...
        if (self.previousCompletetionPercentage < 0.25 && playbackPercent >= 0.25) {
            [self.eventManager trackEvent:SWPBMTrackingEventFirstQuartile];
            SWPBMLogInfo(@"Video Playback Progress: SWPBMTrackingEventFirstQuartile");
        }

        if (self.previousCompletetionPercentage < 0.50 && playbackPercent >= 0.50) {
            [self.eventManager trackEvent:SWPBMTrackingEventMidpoint];
            SWPBMLogInfo(@"Video Playback Progress: SWPBMTrackingEventMidpoint");
        }
        
        if (self.previousCompletetionPercentage < 0.75 && playbackPercent >= 0.75) {
            [self.eventManager trackEvent:SWPBMTrackingEventThirdQuartile];
            SWPBMLogInfo(@"Video Playback Progress: SWPBMTrackingEventThirdQuartile");
        }
...
- (void)trackStartPlaybackEvents {
    [self.eventManager trackEvent:SWPBMTrackingEventNormal];
    ...
    SWPBMLogInfo(@"Video Playback Progress: SWPBMTrackingEventCreativeView/SWPBMTrackingEventStart");
}
        ...
        if (self.avPlayer && self.avPlayer.currentItem) {
            switch (self.avPlayer.status) {
                ...
                case AVPlayerStatusUnknown : {
                    SWPBMLogInfo(@"unknown (This is normal at launch)");
                    return;
                }
                break;
                    
                case AVPlayerStatusFailed: {
                    NSError *error = self.avPlayer.currentItem.error ?: [SWPBMError errorWithDescription:@"Unknown Error"];
...
- (NSNumber *)calculateProgressBarDuration {
    ...
    SWPBMRewardedConfig * rewardedConfig = self.creative.creativeModel.adConfiguration.rewardedConfig;
    ...
    if (ortbPlaybackevent) {
        SWPBMTrackingEvent event = [SWPBMTrackingEventDescription getEventWith:ortbPlaybackevent];
    
        switch(event) {
            case SWPBMTrackingEventStart:
                break;
            case SWPBMTrackingEventFirstQuartile:
                progressBarDuration += 0.25 * videoDuration;
                break;
            case SWPBMTrackingEventMidpoint:
                progressBarDuration += 0.5 * videoDuration;
                break;
            case SWPBMTrackingEventThirdQuartile:
                progressBarDuration += 0.75 * videoDuration;
                break;
            case SWPBMTrackingEventComplete:
