...
@implementation SWPBMORTBAbstract

// MARK: - Public

- (nullable NSString *)toJsonStringWithError:(NSError* _Nullable __autoreleasing * _Nullable)error {
    SWPBMJsonDictionary *jsonDictionary = [self toJsonDictionary];
    return [SWPBMFunctions toStringJsonDictionary:jsonDictionary error:error];
}

+ (instancetype)fromJsonString:(nonnull NSString *)jsonString error:(NSError *__autoreleasing  _Nullable * _Nullable)error {
    SWPBMJsonDictionary* dictionary = [SWPBMFunctions dictionaryFromJSONString:jsonString
                                                                     error:error];
...
- (nonnull SWPBMJsonDictionary *)toJsonDictionary {
    SWPBMLogError(@"You must override %@ in a subclass", NSStringFromSelector(_cmd));
    return [SWPBMJsonDictionary new];
}

- (instancetype)initWithJsonDictionary:(nonnull SWPBMJsonDictionary *)jsonDictionary {
    SWPBMLogError(@"You should not initialize abstract class directly");
    return [SWPBMORTBAbstract new];
}
