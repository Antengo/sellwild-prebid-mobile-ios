...
@implementation NSString(SWPBMExtensions)

-(BOOL) SWPBMdoesMatch: (nonnull NSString *)  regex {
    return [self SWPBMnumberOfMatches: regex];
}

// Return the number matches using a regular expression.
-(int) SWPBMnumberOfMatches: (NSString*)  strRegex {
    if (!strRegex) {
        return 0;
    }
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:strRegex
                                                                           options:0 //NSRegularExpressionCaseInsensitive
                                                                             error:&error];
    if (error) {
        SWPBMLogError(@"Error %@ parsing regex: %@", error.description, strRegex);
        return 0;
    }
...
-(nullable NSString *) SWPBMsubstringToString: (nonnull NSString *) to {
    if (!to) {
        return nil;
    }
...
-(nullable NSString *) SWPBMsubstringFromString: (nonnull NSString *) from {
    if (!from) {
        return nil;
    }
...
//    to: "456"
// returns "XXX"
-(nullable NSString *) SWPBMsubstringFromString:(nonnull NSString *)from toString:(nonnull NSString *) to {
    if (!(to && from)) {
        return nil;
    }
...
-(nonnull NSString *) SWPBMstringByReplacingRegex:(nonnull NSString *) strRegex replaceWith:(nonnull NSString *) replaceWithString {
...
-(nullable NSString *) SWPBMsubstringFromIndex:(int) fromIndex toIndex:(int) toIndex {
