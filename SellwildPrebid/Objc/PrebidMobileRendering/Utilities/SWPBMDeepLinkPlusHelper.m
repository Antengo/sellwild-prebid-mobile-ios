...
#import "SWPBMExternalURLOpeners.h"
#import "SWPBMTrackingURLVisitors.h"
#import "SWPBMURLOpenAttempterBlock.h"
#import "SWPBMDeepLinkPlusHelper+SWPBMExternalLinkHandler.h"
...
@implementation SWPBMDeepLinkPlusHelper
...
+ (SWPBMExternalLinkHandler *)deepLinkPlusHandlerWithExternalLinkHandler:(SWPBMExternalLinkHandler *)externalLinkHandler {
    SWPBMURLOpenAttempterBlock const urlOpenAttempter = ^(NSURL *url,
                                                        SWPBMCanOpenURLResultHandlerBlock compatibilityCheckHandler) {
        if (![self isDeepLinkPlusURL:url]) {
            compatibilityCheckHandler(NO);
            return;
        }
        
        SWPBMExternalURLOpenCallbacks * const callbacks = compatibilityCheckHandler(YES);
        SWPBMURLOpenResultHandlerBlock const completion = callbacks.urlOpenedCallback;
        SWPBMVoidBlock const onClickthroughExitBlock = callbacks.onClickthroughExitBlock;
...
+ (void)tryHandleDeepLinkPlus:(NSURL *)url
          externalLinkHandler:(SWPBMExternalLinkHandler *)externalLinkHandler
                   completion:(void (^)(BOOL visited, NSURL *fallbackURL, NSArray<NSURL *> *trackingURLs))completion
      onClickthroughExitBlock:(nullable SWPBMVoidBlock)onClickthroughExitBlock
{
    SWPBMDeepLinkPlus *deepLinkPlus = [SWPBMDeepLinkPlus deepLinkPlusWithURL:url];
    if (deepLinkPlus == nil) {
        completion(NO, nil, nil);
        return;
    }
    
    SWPBMExternalLinkHandler * const deepLinkHandler = externalLinkHandler.asDeepLinkHandler;
