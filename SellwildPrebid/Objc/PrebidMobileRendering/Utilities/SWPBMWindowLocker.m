...
@implementation SWPBMWindowLocker

- (instancetype)initWithWindow:(UIWindow *)window measurementSession:(SWPBMOpenMeasurementSession *)measurementSession {
    if (!(self = [super init])) {
        return nil;
    }
    _window = window;
    _measurementSession = measurementSession;
    _serialLockerQueue = dispatch_queue_create("SWPBMWindowLocker", DISPATCH_QUEUE_SERIAL);
    return self;
}
...
- (void)lock {
    dispatch_async(self.serialLockerQueue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.locked) {
                SWPBMLogError(@"Attempting to lock already locked window locker.");
                return;
            }
            UIView * lockingView = [self lockingView] ?: [self buildLockingView];
            if (lockingView == nil) {
                SWPBMLogError(@"Failed to create window locking view.");
                return;
            }
...
- (void)unlock {
    dispatch_async(self.serialLockerQueue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!self.locked) {
                SWPBMLogError(@"Attempting to unlock already unlocked window locker.");
                return;
            }
...
- (UIView *)buildLockingView {
    ...
    [self.measurementSession addFriendlyObstruction:backgroundView purpose:SWPBMOpenMeasurementFriendlyObstructionPurposeWindowLockerBackground];
    [self.measurementSession addFriendlyObstruction:activityIndicatorView purpose:SWPBMOpenMeasurementFriendlyObstructionPurposeWindowLockerActivityIndicator];
