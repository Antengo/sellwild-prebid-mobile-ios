...
@class SWPBMCreativeModel;
@class SWPBMMRAIDState;
@protocol SWPBMWebView_Protocol;

NS_ASSUME_NONNULL_BEGIN

typedef void(^SWPBMJSEvaluatingBlock)(NSString * _Nullable command, id _Nullable jsRes, NSError * _Nullable error);

typedef NS_ENUM(NSInteger, SWPBMWebViewState) {
    SWPBMWebViewStateUnloaded,
    SWPBMWebViewStateLoading,
    SWPBMWebViewStateLoaded
};

// The SWPBMWebView is a UIView that contains a WKWebView as a subview.
// The interface of the SWPBMWebView is specific to showing ads within the SDK.
...
@interface SWPBMWebView : UIView <WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, UIGestureRecognizerDelegate, SWPBMWebView_Protocol>
...
@property (nonatomic, copy, nullable) SWPBMJSEvaluatingBlock jsEvaluatingCompletion;

@property (nonatomic, weak, nullable) NSBundle *bundle;
@property (nonatomic, weak, nullable) id<SWPBMWebViewDelegate> delegate;
@property (nonatomic, weak, nullable) id<SWPBMExposureChangeDelegate> exposureDelegate;
@property (nonatomic, strong) SWPBMTouchDownRecognizer *tapdownGestureRecognizer;

@property (nonatomic) SWPBMMRAIDState *mraidState;
@property (nonatomic, assign, readonly) SWPBMWebViewState state;
...
- (void)MRAID_getExpandProperties:(void(^)(SWPBMMRAIDExpandProperties *_Nullable))completionHandler
    NS_SWIFT_NAME(MRAID_getExpandProperties(completionHandler:));

// get the resize properties from mraid.js
- (void)MRAID_getResizeProperties:(void(^)(SWPBMMRAIDResizeProperties *_Nullable))completionHandler
...
- (void)addFriendlyObstructionsToMeasurementSession:(nullable SWPBMOpenMeasurementSession *)session;

#pragma mark - Utilities

// returns a human-readable description of the view's state
+ (nullable NSString *)webViewStateDescription:(SWPBMWebViewState)state;

// checks if the frame is onscreen at all
+ (BOOL)isVisibleView:(nullable UIView *)view;

+ (nullable SWPBMJsonDictionary *)anyToJSONDict:(nullable id)str;
