...
@objc @_spi(SWPBMInternal) public
class SWPBMErrorType: NSObject, RawRepresentable {
    ...
    @objc public static let invalidRequest   = SWPBMErrorType(rawValue: "Invalid request")
    @objc public static let internalError    = SWPBMErrorType(rawValue: "SDK internal error")
    @objc public static let initError        = SWPBMErrorType(rawValue: "Initialization error")
    @objc public static let serverError      = SWPBMErrorType(rawValue: "Server error")
}

@objc @_spi(SWPBMInternal) public
class SWPBMError: NSError, @unchecked Sendable {
    ...
    convenience init(message: String, code: SWPBMErrorCode) {
        self.init(message: message, code: code.rawValue)
    }
    
    convenience init(message: String) {
        self.init(code: SWPBMErrorCode.general.rawValue,
                  userInfo: [
                    NSLocalizedDescriptionKey : message
                  ])
    }
    
    @objc public static func error(message: String, type: SWPBMErrorType) -> SWPBMError {
        SWPBMError(message: "\(type.rawValue): \(message)", code: 0)
    }
    
    @objc public static func error(description: String) -> SWPBMError {
        error(description: description, statusCode: .general)
    }
    
    @objc public static func error(description: String, statusCode: SWPBMErrorCode) -> SWPBMError {
        SWPBMError(message: description, code: statusCode)
    }
    
    @objc @discardableResult
    public static func createError(_ error: UnsafeMutablePointer<NSError?>?,
                                   message: String,
                                   type: SWPBMErrorType) -> Bool {
        if let error {
            let err = SWPBMError.error(message: message, type: type)
    ...
    public static func createError(_ error: UnsafeMutablePointer<NSError?>?,
        ...
        if let error {
            let err = SWPBMError.error(description: description)
            error.initialize(to: err)
            Log.error("\(err)")
            return true
        }
    ...
    public static func createError(_ error: UnsafeMutablePointer<NSError?>?,
                                   description: String,
                                   statusCode: SWPBMErrorCode) -> Bool {
        if let error {
            let err = SWPBMError.error(description: description, statusCode: statusCode)
            error.initialize(to: err)
            Log.error("\(err)")
            return true
        }
    ...
    private static func errorCode(_ subCode: Int, forFamily family: SWPBMErrorFamily) -> Int {
    ...
    class func demandResult(from error: Error?) -> ResultCode {
        ...
        if error.domain == SWPBMError.errorDomain {
