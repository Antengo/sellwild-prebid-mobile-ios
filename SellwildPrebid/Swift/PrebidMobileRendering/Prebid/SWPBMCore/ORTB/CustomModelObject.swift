...
public class CustomModelObjects: NSObject {
    ...
    private static var _customTypes = [(baseType: SWPBMJsonDecodable.Type, customType: SWPBMJsonDecodable.Type)]()
    static func getCustomType<T: SWPBMJsonDecodable>(_ type: T.Type) -> T.Type {
        lockedRead {
            _customTypes.first { $0.baseType == type }?.customType as? T.Type ?? type
        }
    }
    
    static func setCustomType<T: SWPBMJsonDecodable>(baseType: T.Type, customType: T.Type) {
        lockedWrite {
    ...
    static func instantiate<T: SWPBMJsonDecodable>(json: [String : Any]) -> T? {
        getCustomType(T.self).init(jsonDictionary: json)
    }
    
    public static func unregisterCustomType(_ type: SWPBMJsonDecodable.Type) {
        lockedWrite {
