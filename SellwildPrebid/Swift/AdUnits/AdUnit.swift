...
public class AdUnit: NSObject, DispatcherDelegate {
    ...
    public init(configId: String, size: CGSize?, adFormats: Set<AdFormat>) {
        ...
        SellwildPrebid.shared.useCacheForReportingWithRenderingAPI = true
    }
    ...
    func baseFetchDemand(
        ...
        if SellwildPrebid.shared.prebidServerAccountId.isEmpty || SellwildPrebid.shared.prebidServerAccountId.containsOnly(.whitespaces) {
        ...
        let timeout = Int(truncating: SellwildPrebid.shared.timeoutMillisDynamic ?? NSNumber(value: .PB_Request_Timeout))
