...
class SWPBMCreativeFactoryJobTest: XCTestCase {
    ...
    func testVastCreativeFail() {
        ...
        let finishedCallback = { (job: SWPBMCreativeFactoryJob, error: Error?) in
            if error != nil {
                XCTAssertEqual(error?.localizedDescription ?? "", "SWPBMCreativeFactoryJob: Could not initialize VideoCreative without videoFileURL")
                expectationFailure.fulfill()
            }
        }
        
        let job = SWPBMCreativeFactoryJob(from: model, transaction: transaction, serverConnection: connection, finishedCallback: finishedCallback)
        
        job.state = SWPBMCreativeFactoryJobStateRunning
    ...
    func testTimerExpiring() {
        ...
        let finishedCallback = { (job: SWPBMCreativeFactoryJob, error: Error?) in
            if error != nil {
                XCTAssertEqual(error?.localizedDescription ?? "", "SDK internal error: SWPBMCreativeFactoryJob: Failed to complete in specified time interval")
                expectationFailure.fulfill()
            }
        }
    ...
    func testStartWithWrongState() {
        ...
        let finishedCallback = { (job: SWPBMCreativeFactoryJob, error: Error?) in
            if error != nil {
                XCTAssertEqual(error?.localizedDescription ?? "", "SDK internal error: SWPBMCreativeFactoryJob: Tried to start SWPBMCreativeFactory twice")
                expectationFailure.fulfill()
            }
        }
    ...
    func testCreativeDownloadDelegateSuccess() {
        ...
        let finishedCallback = { (job: SWPBMCreativeFactoryJob, error: Error?) in
            if job.state == SWPBMCreativeFactoryJobStateSuccess && error == nil {
        ...
        waitForExpectations(timeout: 5, handler: { _ in
            XCTAssertEqual(job.state, SWPBMCreativeFactoryJobStateSuccess)
        })
    }
    ...
    func testCreativeDownloadDelegateFailure() {
        ...
        job.state = SWPBMCreativeFactoryJobStateRunning
        job.failWithError(SWPBMError.error(description: ""))
        
        waitForExpectations(timeout: 5, handler: { _ in
            XCTAssertEqual(job.state, SWPBMCreativeFactoryJobStateError)
        })
    }
    ...
    func testTimeoutInterval_apiProvided() {
        ...
        SellwildPrebid.shared.creativeFactoryTimeout = creativeFactoryTimeout
        SellwildPrebid.shared.creativeFactoryTimeoutPreRenderContent = creativeFactoryTimeoutPreRenderContent
