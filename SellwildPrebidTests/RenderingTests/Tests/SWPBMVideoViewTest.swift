...
class SWPBMVideoViewTest: XCTestCase, CreativeResolutionDelegate, CreativeViewDelegate, SWPBMVideoViewDelegate {
    ...
    func testLearnMoreButton() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5, execute: { [weak self] in
            ...
            guard let videoView = strongSelf.videoCreative.view as? SWPBMVideoView else {
    ...
    // MARK: - SWPBMCreativeDownloadDelegate
    ...
    private func setupVideoCreative(videoFileURL:String = "http://get_video/small.mp4", localVideoFileName:String = "small.mp4", adConfiguration: AdConfiguration = AdConfiguration()) {
        ...
        downloader.downloadData(for: url, maxSize: SWPBMVideoCreative.maxSizeForPreRenderContent, completionClosure: { (data:Data?, error:Error?) in
