...
class SWPBMMRAIDControllerTest_Base: XCTestCase, CreativeViewDelegate {
    
    let timeout: TimeInterval = 1
    
    var serverConnection: PrebidServerConnection!
    var transaction: Transaction!
    var mockHtmlCreative: MockSWPBMHTMLCreative!
    var mockCreativeModel: MockSWPBMCreativeModel!
    var mockEventTracker: MockSWPBMAdModelEventTracker!
    var mockModalManager: MockModalManager!
    var mockWebView: MockSWPBMWebView!
    var mockViewController: MockViewController!
    
    var MRAIDController: SWPBMMRAIDController!
    ...
    override func setUp() {
        ...
        self.mockCreativeModel = MockSWPBMCreativeModel(adConfiguration: AdConfiguration())
        self.mockCreativeModel.width = 320
        self.mockCreativeModel.height = 50
        self.mockCreativeModel.html = "test"
        
        self.mockEventTracker = MockSWPBMAdModelEventTracker(creativeModel: self.mockCreativeModel,
        ...
        self.mockWebView = MockSWPBMWebView()
        self.mockHtmlCreative = MockSWPBMHTMLCreative(
        ...
        //Simulate SWPBMBannerView.creativeReadyForImmediateDisplay:
        //Add the view to the "SWPBMBannerView" (in this case, the viewController's view)
    ...
    // MARK: - Utilities
    /**
     Setup an expectation and associated, mocked `SWPBMWebView` to fulfill that expectation.
    ...
    func mraidErrorExpectation(shouldFulfill: Bool, message expectedMessage: String? = nil, action expectedAction: SWPBMMRAIDAction? = nil, file: StaticString = #file, line: UInt = #line) {
        ...
        self.mockWebView.mock_MRAID_error = { (actualMessage, actualAction) in
            if shouldFulfill {
                SWPBMAssertEq(actualMessage, expectedMessage, file: file, line: line)
                SWPBMAssertEq(actualAction, expectedAction, file: file, line: line)
            }
            exp.fulfill()
        }
    }
    
    func createLoader(connection: PrebidServerConnectionProtocol) -> SWPBMCreativeFactoryDownloadDataCompletionClosure {
        let result: SWPBMCreativeFactoryDownloadDataCompletionClosure =  {url, completionBlock in
            let downloader = SWPBMDownloadDataHelper(serverConnection:connection)
