...
class SWPBMWebViewTest : XCTestCase, SWPBMWebViewDelegate {
    ...
    func testRequiredProperties() {
        let webView = SWPBMWebView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
    ...
    func testIsVisible() {
        ...
        XCTAssertFalse(SWPBMWebView.isVisibleView(nil))
        XCTAssertFalse(SWPBMWebView.isVisibleView(testView))
        XCTAssertFalse(SWPBMWebView.isVisibleView(parentWindow))
        ...
        XCTAssertTrue(SWPBMWebView.isVisibleView(testView))
    ...
    func testIsVisible2() {
        ...
        XCTAssertTrue(SWPBMWebView.isVisibleView(root))
        XCTAssertTrue(SWPBMWebView.isVisibleView(grandparent))
        XCTAssertTrue(SWPBMWebView.isVisibleView(parent))
        XCTAssertTrue(SWPBMWebView.isVisibleView(view))
        ...
        XCTAssertTrue(SWPBMWebView.isVisibleView(grandparent))
        XCTAssertTrue(SWPBMWebView.isVisibleView(parent))
        XCTAssertFalse(SWPBMWebView.isVisibleView(view))
        ...
        XCTAssertTrue(SWPBMWebView.isVisibleView(root))
        XCTAssertTrue(SWPBMWebView.isVisibleView(grandparent))
        XCTAssertFalse(SWPBMWebView.isVisibleView(parent))
        XCTAssertTrue(SWPBMWebView.isVisibleView(view))
    ...
    func testAnyToJSONDict() {
        ...
        //Test valid input
        XCTAssertNotNil(SWPBMWebView.any(toJSONDict: "{}"))
        XCTAssertNotNil(SWPBMWebView.any(toJSONDict: "{\"test\" : {\"key\" : \"value\", \"vlue\" : \"key\"}}"))
        
        //Confirm that invalid input will result in a nil
        XCTAssertNil(SWPBMWebView.any(toJSONDict: nil))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: UIView()))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: [""]))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: ["":""]))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: ""))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: "test"))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: "test{}"))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: "⛵"))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: "Ũ"))
        XCTAssertNil(SWPBMWebView.any(toJSONDict: "[]"))
    }
    
    func testWebViewStateDescription() {
        let cases: [SWPBMWebViewState] = [.unloaded, .loading,  .loaded]
        
        cases.forEach {
            let description = SWPBMWebView.webViewStateDescription($0)
    ...
    func testChangeToMRAIDState() {
        let webView = SWPBMWebView(frame: CGRect(x: 0.0, y: 0.0, width: 100.0, height: 100.0))
    ...
    func testExpand() {
        ...
        // Older OSes fire [SWPBMWebView webView:didFinishNavigation:] earlier
        // than we are expecting.
        let osVersion = Int(UIDevice.current.systemVersion.split(separator: ".")[0])!
        let expectedState = (osVersion >= 10) ? SWPBMWebViewState.loading.rawValue : SWPBMWebViewState.loaded.rawValue
    ...
    func testExpandGlobalQueue() {
        ...
        class MockWebView: SWPBMWebView {
            
            var thread: SWPBMThread?
        ...
        let mockedThread = SWPBMThread { isCalledFromMainThread in
    ...
    func testMRAID_error() {
        ...
        webView.MRAID_error("test error", action: SWPBMMRAIDAction.log)
        
        let log = Log.getLogFileAsString() ?? ""
        
        XCTAssertTrue(log.contains("Action: [\(SWPBMMRAIDAction.log.rawValue)] generated error with message [test error]"))
    }
    ...
    private func runTestPrepareForMRAIDWithWindow(coppaValue:Int, coppaFlag:Bool) {
        ...
        let webView = SWPBMWebView(frame: frame, creativeModel: nil, targeting: targeting)
        webView.delegate = self
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand == SWPBMMRAIDJavascriptCommands.onExposureChange(Factory.createViewExposure(exposureFactor: 1, visibleRectangle: webView.bounds)) {
    ...
    func testMRAID_onViewableChangeTrue() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == SWPBMMRAIDJavascriptCommands.onViewableChange(true)
        }
    ...
    func testMRAID_onExposureChangeTrue() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == SWPBMMRAIDJavascriptCommands.onExposureChange(Factory.createViewExposure(exposureFactor: 1, visibleRectangle: webView.bounds))
        }
    ...
    func testMRAID_updatePlacementType() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == SWPBMMRAIDJavascriptCommands.updatePlacementType(.inline)
        }
    ...
    func testAudioVolumeChange() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == SWPBMMRAIDJavascriptCommands.onAudioVolumeChange(50.0)
        }
    ...
    func testGetCurrentAppOrientation() {
        ...
        let webView = SWPBMWebView(frame: frame)
        webView.delegate = self
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand ==  SWPBMMRAIDJavascriptCommands.updateCurrentAppOrientation(orientationStr,
                                                                                    locked: false){
                self.expectationCommandExecuted?.fulfill()
            }
        }
    ...
    func testUIApplicationDidChangeStatusBarOrientationNotification() {
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand == SWPBMMRAIDJavascriptCommands.updateMaxSize(SWPBMFunctions.deviceMaxSize()) {
                self.expectationCommandExecuted?.fulfill();
            }
        }
    ...
    func testLayoutSubviews() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            let res = SWPBMMRAIDJavascriptCommands.updateCurrentPosition(webView.frame)
    ...
    // MARK: - SWPBMWebViewDelegate
    
    func viewControllerForPresentingModals() -> UIViewController? {
        return UIViewController()
    }
    
    func webViewReady(toDisplay webView: SWPBMWebView) {
        expectationWebViewReadyToDisplay?.fulfill()
        readyToDisplayBlock?()
    }
    
    func webView(_ webView: SWPBMWebView, failedToLoadWithError error: Error) {
        expectationWebViewFailed?.fulfill()
    }
    
    func webView(_ webView: SWPBMWebView, receivedClickthroughLink url: URL) {
        expectationWebViewShouldOpenExternalLink?.fulfill()
    }
    
    func webView(_ webView: SWPBMWebView, receivedMRAIDLink url: URL) {
        expectationWebViewReceivedMRAIDLink?.fulfill()
    }
    
    func webView(_ webView: SWPBMWebView, receivedRewardedEventLink url: URL) {}
    
    // MARK: - Check methods
    
    private func checkJSEvaluating( webView: SWPBMWebView,
