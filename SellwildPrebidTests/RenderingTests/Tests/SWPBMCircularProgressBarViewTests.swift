...
class SWPBMCircularProgressBarViewTests: XCTestCase {
    
    func testInit() {
        var circularProgressBarView = SWPBMCircularProgressBarView()
        XCTAssertNotNil(circularProgressBarView)
        
        let viewRect = CGRect(x: 0, y: 0, width: 50, height: 50)
        circularProgressBarView = SWPBMCircularProgressBarView(frame: viewRect)
        XCTAssertNotNil(circularProgressBarView)
    }
    ...
    func testProperties() {
        ...
        let circularProgressBarView = SWPBMCircularProgressBarView()
        XCTAssertNotNil(circularProgressBarView)
    ...
    func testDataObject() {
        let data = NSKeyedArchiver.archivedData(withRootObject: SWPBMCircularProgressBarView())
        XCTAssertNotNil(data)
        let object = NSKeyedUnarchiver.unarchiveObject(with: data)
        XCTAssert(object is SWPBMCircularProgressBarView)
    }
}
