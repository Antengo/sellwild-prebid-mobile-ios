...
class SWPBMURLComponentsTest: XCTestCase {
    
    //Demonstrate SWPBMURLComponents will overwrite and append key-val pairs, then sort them.
    func testPositive() {
        let traceParamDict = ["overwritekey" : "overwriteval", "appendkey" : "appendval"]
        let strURL = "https://foo.com/?overwritekey=bar"
        
        let urlComponents = SWPBMURLComponents.init(url:strURL, paramsDict:traceParamDict)!
