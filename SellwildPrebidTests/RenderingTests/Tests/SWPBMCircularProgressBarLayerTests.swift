...
class SWPBMCircularProgressBarLayerTests: XCTestCase {
    
    func testNeedsDisplayForKey() {
        XCTAssertTrue(SWPBMCircularProgressBarLayer.needsDisplay(forKey: "value"))
        XCTAssertFalse(SWPBMCircularProgressBarLayer.needsDisplay(forKey: "nonNeedsDisplay"))
    }
    
    func testProperties() {
        let circularProgressBarView = SWPBMCircularProgressBarView(frame: CGRect(x: 0, y: 0, width: 50, height: 50))
        XCTAssertNotNil(circularProgressBarView)
        
        guard let circularProgressBarLayer = circularProgressBarView.layer as? SWPBMCircularProgressBarLayer else {
            XCTFail()
            return
        }
    ...
    func testUpdateProgress() {
        ...
        let circularProgressBarView = SWPBMCircularProgressBarView(frame: viewRect)
        XCTAssertNotNil(circularProgressBarView)
