...
class SWPBMHTMLCreativeTest_MRAIDExpand: SWPBMHTMLCreativeTest_Base {
    ...
    func testInvalidView() {
        ...
        UtilitiesForTesting.executeTestClosure({
            htmlCreative.webView(mockWebView, receivedMRAIDLink:UtilitiesForTesting.getMRAIDURL("expand"))
        }, checkLogFor:["Could not cast creative view to SWPBMWebView"])
    ...
    func testNoExpandSuccess() {
        ...
        mockModalManager.mock_pushModalClosure = { [weak self] (state, fromRootViewController, animated, shouldReplace, completionHandler) in
            SWPBMAssertEq(state.view, self?.mockWebView)
    ...
    /**
     Setup an expectation and associated, mocked `SWPBMCreativeModel` to fulfill that expectation.
    ...
    func clickTrackingExpectation(shouldFulfill: Bool, expectedEvent: TrackingEvent? = nil) {
        ...
        mockEventTracker.mock_trackEvent = { (actualEvent) in
            if shouldFulfill {
                SWPBMAssertEq(actualEvent, expectedEvent)
            }
            exp.fulfill()
        }
    }
    ...
    func mraidStateChangeExpectation(shouldFulfill: Bool, exptectedState: MRAIDState) {
        ...
        mockWebView.mock_changeToMRAIDState = { (actualState) in
            SWPBMAssertEq(actualState, exptectedState)
            exp.fulfill()
        }
    }
