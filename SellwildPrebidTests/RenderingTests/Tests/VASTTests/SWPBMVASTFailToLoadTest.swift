...
class SWPBMVASTFailToLoadTest: XCTestCase, SWPBMAdLoadManagerDelegate {
    ...
    func loadAdLoadManager(file: String) {
        ...
        let adLoadManager = SWPBMAdLoadManagerVAST(bid: RawWinningBidFabricator.makeWinningBid(price: 0.1, bidder: "bidder", cacheID: "cache-id"), connection: conn, adConfiguration: AdConfiguration())
    ...
    //MARK: SWPBMAdLoadManagerDelegate
    
    func loadManager(_ loadManager: SWPBMAdLoadManagerProtocol, didLoad transaction: Transaction) {
        XCTFail()
    }
    
    func loadManager(_ loadManager: SWPBMAdLoadManagerProtocol, failedToLoad transaction: Transaction?, error: Error) {
