...
class SWPBMBidResponseTransformerTest: XCTestCase {
    
    func testInvalidAccountID() {
        let response = SWPBMBidResponseTransformer.invalidAccountIDResponse(accountID: "0689a263-318d-448b-a3d4-b02e8a709d9d")
        
        do {
            let _ = try SWPBMBidResponseTransformer.transform(response)
    ...
    func testInvalidConfigId() {
        let response = SWPBMBidResponseTransformer.invalidConfigIdResponse(configId: "b6260e2b-bc4c-4d10-bdb5-d1c2b6c0c97a")
        
        do {
            let _ = try SWPBMBidResponseTransformer.transform(response)
            XCTFail("Expected error not thrown")
        } catch {
            XCTAssertEqual(error as NSError, SWPBMError.prebidInvalidConfigId() as NSError)
        }
    }
    
    func testInvalidSize() {
        let response = SWPBMBidResponseTransformer.invalidSizeResponse(impIndex: 0, formatIndex: 0)
        
        do {
            let _ = try SWPBMBidResponseTransformer.transform(response)
            XCTFail("Expected error not thrown")
        } catch {
            XCTAssertEqual(error as NSError, SWPBMError.prebidInvalidSize() as NSError)
        }
    }
    
    func testServerError() {
        let messageBody = "Invalid request: some server reason, probably"
        let response = SWPBMBidResponseTransformer.serverErrorResponse
        
        do {
            let _ = try SWPBMBidResponseTransformer.transform(response)
            XCTFail("Expected error not thrown")
        } catch {
            XCTAssertEqual(error as NSError, SWPBMError.serverError(messageBody) as NSError)
        }
    }
    
    func testNoJsonDic() {
        let response = SWPBMBidResponseTransformer.nonJsonDicResponse
        
        do {
            let _ = try SWPBMBidResponseTransformer.transform(response)
            XCTFail("Expected error not thrown")
        } catch {
            XCTAssertEqual(error as NSError, SWPBMError.jsonDictNotFound() as NSError)
        }
    }
    
    func testOk() {
        let bidPrice: Float = 0.1091000000051168
        let response = SWPBMBidResponseTransformer.makeValidResponse(bidPrice: bidPrice)
        
        let bidResponse = try! SWPBMBidResponseTransformer.transform(response)
    ...
    func testZeroPriceBid() {
        let response = SWPBMBidResponseTransformer.makeValidResponse(bidPrice: 0)
    ...
    func testRealPrebidResponse() {
        ...
        let serverResponse = SWPBMBidResponseTransformer.buildResponse(realResponseBody)
        let response = try! SWPBMBidResponseTransformer.transform(serverResponse)
    ...
    func testMacroReplacement() {
        ...
        let serverResponse = SWPBMBidResponseTransformer.buildResponse(responseBody)
        let response = try! SWPBMBidResponseTransformer.transform(serverResponse)
