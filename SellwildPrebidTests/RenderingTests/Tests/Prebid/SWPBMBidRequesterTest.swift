...
class SWPBMBidRequesterTest: XCTestCase {
    ...
    func testBanner_requireServerCacheAndNoCachedBids_returnsNoBidsError() {
        ...
        let adUnitConfig = AdUnitConfig(configId: configId, size: CGSize(width: 300, height: 250))
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            callback(SWPBMBidResponseTransformer.makeValidResponse(bidPrice: 0.1))
        }])
        let requester = Factory.createBidRequester(connection: connection,
                                                   sdkConfiguration: sdkConfiguration,
                                                   targeting: targeting,
        ...
        requester.requestBids { (bidResponse, error) in
            ...
            XCTAssertEqual(error as NSError?, SWPBMError.noCachedBids() as NSError?)
            exp.fulfill()
        }
    ...
    func testBanner_requireServerCacheAndNoBids_returnsNoWinningBidError() {
        ...
        let adUnitConfig = AdUnitConfig(configId: configId, size: CGSize(width: 300, height: 250))
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            callback(SWPBMBidResponseTransformer.noWinningBidResponse)
        }])
        let requester = Factory.createBidRequester(connection: connection,
                                                   sdkConfiguration: sdkConfiguration,
                                                   targeting: targeting,
        ...
        requester.requestBids { (bidResponse, error) in
            XCTAssertNil(bidResponse)
            XCTAssertEqual(error as NSError?, SWPBMError.noWinningBid() as NSError?)
            exp.fulfill()
        }
    ...
    func testBanner_requireServerCacheAndCachedBid_returnsBidResponse() {
        ...
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            ...
            callback(SWPBMBidResponseTransformer.buildResponse(responseBody))
        }])
        let requester = Factory.createBidRequester(connection: connection,
                                                   sdkConfiguration: sdkConfiguration,
                                                   targeting: targeting,
    ...
    func testBanner_invalidAccountID_rejectedByServer() {
        ...
        let adUnitConfig = AdUnitConfig(configId: configId, size: CGSize(width: 300, height: 250))
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            callback(SWPBMBidResponseTransformer.invalidAccountIDResponse(accountID: accountID))
        }])
        let requester = Factory.createBidRequester(connection: connection,
                                                   sdkConfiguration: sdkConfiguration,
                                                   targeting: targeting,
    ...
    func testBanner_invalidConfigID_rejectedByServer() {
        ...
        let adUnitConfig = AdUnitConfig(configId: configId, size: CGSize(width: 300, height: 250))
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            callback(SWPBMBidResponseTransformer.invalidConfigIdResponse(configId: configId))
        }])
        let requester = Factory.createBidRequester(connection: connection,
                                                   sdkConfiguration: sdkConfiguration,
                                                   targeting: targeting,
    ...
    func testConcurrentDuplicateCallbacks_ShouldBeThreadSafe() {
        ...
        let connection = MockServerConnection(onPost: [{ (url, data, timeout, callback) in
            let response = SWPBMBidResponseTransformer.someValidResponse
