...
// Because the SWPBMAbstractCreative_Objc's conformance to SWPBMAbstractCreative is declared in
// objc with a forward declaration, the compiler doesn't let us call SWPBMAbstractCreative methods
// on instances of SWPBMAbstractCreative_Objc without an explicit cast.
// As a workaround, we explicitly redeclare all of the protocol's methods here -- except for those
// that are declared in SWPBMAbstractCreative+PbmTestExtension.h to allow for subclass overrides.
extension SWPBMAbstractCreative_Objc {
    ...
    var dismissInterstitialModalState: SWPBMVoidBlock { p.dismissInterstitialModalState }
